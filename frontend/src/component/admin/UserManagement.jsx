...
const UserManagement = () => {
  ...
  const { users, loading, error } = useSelector((state) => state.admin) || { users: [] }; // Ensure users is an array
  ...
  {users && users.length > 0 ? (
    users
      .filter((user) => user && user.name && user.email) // Remove users with missing data
      .map((user) => (
